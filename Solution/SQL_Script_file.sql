...
SELECT COUNT(*) AS No_of_Row FROM director_mapping;

-- Rows in genre table
SELECT COUNT(*) AS No_of_Row FROM genre;

-- Rows in movies table
SELECT COUNT(*) AS No_of_Row FROM movie;

-- Rows in names table
SELECT COUNT(*) AS No_of_Row FROM names;
    
-- Rows in ratings table
SELECT COUNT(*) AS No_of_Row FROM ratings;

-- Rows in role_mapping table
SELECT COUNT(*) AS No_of_Row FROM role_mapping;
...
SELECT 
    year,
    COUNT(title) AS No_of_movies_in_2019
		FROM 
			movie
WHERE 
	country = 'USA' OR 
...
SELECT
	genre AS Genre, 
	COUNT(movie_id) AS No_of_movies_produced 
		FROM 
			genre
GROUP BY 
	Genre
ORDER BY 
	No_of_movies_produced DESC
LIMIT 1;
...
SELECT 
	...
	DENSE_RANK()
		OVER( ORDER BY 
				COUNT(m.id) DESC) AS prod_company_rank 
FROM 
...
SELECT 
	...
	COUNT(g.movie_id) AS movie_count,
	ROW_NUMBER() OVER(ORDER BY COUNT(g.movie_id) DESC) AS directors_rank
FROM 
	names AS n INNER JOIN director_mapping AS dm 
			ON n.id = dm.name_id 
	...
	top3_genre
WHERE 
	g.genre in (top3_genre.genre) AND 
    avg_rating > 8
GROUP BY 
	director_name
ORDER BY 
...
SELECT 
	...
	DENSE_RANK() 
		OVER(ORDER BY avg_rating DESC) AS actor_rank
FROM 
	movie m 
		INNER JOIN ratings r 
			ON m.id=r.movie_id
...
SELECT 
	...
	DENSE_RANK() 
		OVER(ORDER BY avg_rating DESC) AS actress_rank
FROM 
	movie m 
		INNER JOIN ratings r 
			ON m.id=r.movie_id
...
(
	SELECT 
		genre,
		year, 
		title AS movie_name, worlwide_gross_income,
		DENSE_RANK() OVER(
				PARTITION BY year ORDER BY worlwide_gross_income DESC) AS movie_rank
...
FROM 
	names n INNER JOIN role_mapping rm 
				ON n.id = rm.name_id
			INNER JOIN ratings r 
				ON rm.movie_id = r.movie_id
			INNER JOIN genre g 
